...
export default function App() {
  ...
  const setSafeStatus = (message) => {
    ...
    window.clearTimeout(window.__sabGStatusTimer);
    window.__sabGStatusTimer = window.setTimeout(() => {
      setStatusMessage("");
    }, 2500);
  };
  ...
  if (authLoading) {
    return (
      <div style={styles.page}>
        <h1 style={styles.heading}>sabG Admin Panel</h1>
        <p>Loading session...</p>
      </div>
    );
  }
  ...
  return (
    <div style={styles.page}>
        ...
        {filteredOrders.map((o) => (
          <div key={o.id} style={styles.orderCard}>
            <div style={{ flex: 1 }}>
              ...
              <div><strong>Delivery Fee:</strong> ₹{Number(o.delivery_fee || 0).toFixed(2)}</div>
              <div><strong>Platform Fee:</strong> ₹{Number(o.platform_fee || 0).toFixed(2)}</div>
              ...
              <div><strong>Promo Code:</strong> {o.promo_code || "-"}</div>
      ...
      <h2>Offers &amp; Promo Codes ({offers.length})</h2>
      <div style={styles.list}>
        {offers.length === 0 ? <p style={{opacity:0.6}}>No offers yet.</p> : offers.map(o => (
          <div key={o.id} style={styles.listItem}>
            <div>
              <strong style={{fontSize:16,letterSpacing:1}}>{o.code}</strong>
              <div style={styles.smallText}>{o.type === "percent" ? `${o.discount}% off` : `₹${o.discount} off`}</div>
              <div style={styles.smallText}>Min order: ₹{o.min_order || 0} · {o.active ? "✅ Active" : "❌ Inactive"}</div>
            </div>
            <button style={styles.buttonDanger} onClick={async () => {
      ...
      <div style={styles.list}>
        {banners.map(b => (
          <div key={b.id} style={{...styles.listItem, background: "transparent", border:"1px solid #334155"}}>
            <div style={{flex:1}}>
              <div style={{background: b.bg || "#16a34a", borderRadius:10, padding:"12px 16px", marginBottom:8}}>
                <p style={{margin:"0 0 2px",color:"rgba(255,255,255,0.8)",fontSize:12}}>{b.subtitle}</p>
                <strong style={{color:"#fff",fontSize:16}}>{b.title}</strong>
              </div>
              <div style={styles.smallText}>{b.active ? "✅ Visible on app" : "❌ Hidden"}</div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <button style={styles.buttonSecondary} onClick={async () => {
                await supabase.from("banners").update({active: !b.active}).eq("id", b.id);
                fetchBanners();
              }}>{b.active ? "Hide" : "Show"}</button>
              <button style={styles.buttonDanger} onClick={async () => {
...
function OfferForm({ fetchOffers }) {
  const [form, setForm] = React.useState({ code:"", discount:"", type:"percent", min_order:"", description:"", active:true });
  const [saving, setSaving] = React.useState(false);

  const save = async () => {
    if (!form.code.trim() || !form.discount) { alert("Code and discount required."); return; }
    setSaving(true);
    ...
    setForm({ code:"", discount:"", type:"percent", min_order:"", description:"", active:true });
    fetchOffers();
    alert("Offer created!");
  };
  ...
  return (
    <div style={styles.section}>
      <input style={styles.input} placeholder="Promo Code (e.g. SAVE20)" value={form.code} onChange={e => setForm(p=>({...p,code:e.target.value.toUpperCase()}))} />
      <input style={styles.input} placeholder="Discount Value" type="number" value={form.discount} onChange={e => setForm(p=>({...p,discount:e.target.value}))} />
      <select style={styles.input} value={form.type} onChange={e => setForm(p=>({...p,type:e.target.value}))}>
        <option value="percent">% Percent Off</option>
        <option value="flat">₹ Flat Off</option>
      </select>
      <input style={styles.input} placeholder="Min Order (₹)" type="number" value={form.min_order} onChange={e => setForm(p=>({...p,min_order:e.target.value}))} />
      <input style={styles.input} placeholder="Description (optional)" value={form.description} onChange={e => setForm(p=>({...p,description:e.target.value}))} />
      <label style={styles.checkboxRow}>
        <input type="checkbox" checked={form.active} onChange={e => setForm(p=>({...p,active:e.target.checked}))} />
        Active
      </label>
      <button style={styles.button} onClick={save} disabled={saving}>{saving ? "Saving..." : "Create Offer"}</button>
    </div>
  );
}
...
function BannerForm({ fetchBanners }) {
  ...
  const [form, setForm] = React.useState({ title:"", subtitle:"", bg:GRADIENTS[0], active:true });
  const [saving, setSaving] = React.useState(false);
  ...
  const save = async () => {
    ...
    setSaving(false);
    if (error) { alert(error.message); return; }
    setForm({ title:"", subtitle:"", bg:GRADIENTS[0], active:true });
    fetchBanners();
  ...
  return (
    <div>
      <div style={{...styles.section, marginBottom:12}}>
        <input style={styles.input} placeholder="Banner Title" value={form.title} onChange={e => setForm(p=>({...p,title:e.target.value}))} />
        <input style={styles.input} placeholder="Subtitle (e.g. Up to 40% off)" value={form.subtitle} onChange={e => setForm(p=>({...p,subtitle:e.target.value}))} />
        <label style={styles.checkboxRow}><input type="checkbox" checked={form.active} onChange={e => setForm(p=>({...p,active:e.target.checked}))} />Show on App</label>
        <button style={styles.button} onClick={save} disabled={saving}>{saving ? "Saving..." : "Add Banner"}</button>
      </div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
        {GRADIENTS.map(g => (
          <div key={g} onClick={() => setForm(p=>({...p,bg:g}))} style={{width:40,height:40,background:g,borderRadius:8,cursor:"pointer",border:form.bg===g?"3px solid #fff":"3px solid transparent"}} />
        ))}
      </div>
      {form.title && (
        <div style={{background:form.bg,borderRadius:12,padding:"16px 20px",marginBottom:12}}>
          <p style={{margin:"0 0 4px",color:"rgba(255,255,255,0.8)",fontSize:12}}>{form.subtitle||"Subtitle preview"}</p>
          <strong style={{color:"#fff",fontSize:18}}>{form.title}</strong>
        </div>
      )}
    </div>
  );
